...
export default function TabBar() {
  ...
  const badgeCount = (tab: Tab) => {
    if (tab.to === '/') return totalUnread
    if (tab.to === '/friends') return pendingCount
    return 0
  }

  return (
    <nav className="z-10 border-t border-white/5 bg-ink-900/70 px-1 pb-[env(safe-area-inset-bottom)] backdrop-blur-xl">
                  ...
                  <>
                    <span
                      className={`absolute -top-px h-0.5 rounded-full bg-papaya-400 transition-all duration-300 ${
                        isActive ? 'w-6 opacity-100 shadow-glow' : 'w-0 opacity-0'
                      }`}
                    />
                    <span
